...
export function LibraryBookCard({ book, readingProgressMap, onLongPress, onPress }) {
  ...
  const statusLabel = isRead ? 'Прочитано' : isReading ? 'Читаю' : '';
    ...
    <View style={styles.card}>
      <TouchableOpacity activeOpacity={0.88} onPress={onPress} delayLongPress={350} onLongPress={onLongPress}>
        <Image
          ...
          style={styles.cover}
        />
        <Text style={styles.title} numberOfLines={2}>{book?.title || 'Без назви'}</Text>
        ...
        <View style={styles.progressRow}>
          <View style={{ flexDirection:'row', alignItems:'center' }}>
            ...
            <Text style={[styles.percent, { color: statusColor }]}>{percentText}</Text>
          </View>
          {!!statusLabel && (
        ...
        {/* progress bar removed: show icon + percent + badge only */}
      </TouchableOpacity>
    </View>
  );
}
